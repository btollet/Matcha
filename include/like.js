...
async function check_like(name, bdd, res, sess, notif) {
    ...
    if (user_exist) {
        ...
        if (is_like) {
            ...
            await bdd.collection('notification').insertOne({ login: name, mes: sess.login + ' ne vous like plus', vue: false });
            notif.emit('messages', name)
            res.end('unlike')
        }
        else {
            bdd.collection('like').insertOne({ login: sess.login, like: name })
            await bdd.collection('notification').insertOne({ login: name, mes: sess.login + ' vous like', vue: false });
            notif.emit('messages', name)
            res.end('like')
        }
    }
