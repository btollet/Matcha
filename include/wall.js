    ...
    if (fake) {
        ...
        await Promise.all(my_tag.map(async (val) => {
            ...
            await Promise.all(user_tag.map(async (user) => {
                let user_info = await bdd.collection('users').findOne({ login: user.login, fake: { $lt: 10 } } )
                if (user_info) {
                    let is_bloque = await bdd.collection('bloque').findOne({ login: sess.login, bloque: user.login })
                    if (!is_bloque) {
